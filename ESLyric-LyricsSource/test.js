        ...
        if (song && !song.code) {
            ...
            for (i = 0; i < song.data.song.list.length && count < QM_CFG.L_MAX; i++) {
                ...
                if (lyric && !lyric.code) {
                    ...
                    if (!lrc_order.length) lrc_order = ["newtype", "origin"];
                    for (var key in lrc_order) {
                        switch (lrc_order[key]) {
                            ...
                            case "newtype":
                                if ((lyric.lyric.length > 128) && (lyric.trans.length > 128)) {
                                    new_lyric.LyricText = lrc_newtype(Base64.decode(lyric.lyric), Base64.decode(lyric.trans));
...
function lrc_newtype(olrc, tlrc) {
    olrc = olrc.split("\n");
    tlrc = tlrc.split("\n");
